...
def main():
    services = ["broker", "controller"]
    parser = ArgumentParser(description="Creates Rucio Kafka Resources")
    parser.add_argument(
        "--svc",
        action="store",
        help="The service (broker, controller, ...) for who storageclass,pv,pvc config must be generated",
        default="all",
    )
    parser.add_argument(
        "--config",
